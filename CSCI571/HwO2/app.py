...
def create_app():
  ...
  blood_pressure_predictor = Blueprint('blood_pressure_predictor', __name__, template_folder="templates", static_folder='static')
  ...
  def predict():
    ...
    if clf is None:
      ...
      return jsonify({
        'status': "failed",
        "code": 404,
        "error": "Prediction model could not be found!"
      })
    ...
    return jsonify({
      ...
      'result': {
        'blood_pressure': clf.predict(x)[0]
      }
    })
